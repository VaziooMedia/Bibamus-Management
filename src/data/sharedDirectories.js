...
export async function loadAppUsers() {
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, email, name, last_name, nickname, gender, gender_custom, bibro_code, birth_date, avatar_emoji, country, city, locality, phone, facebook_url, whatsapp_url, instagram_url, tiktok_url, snapchat_url, x_url, threads_url, linkedin_url, pinterest_url, twitch_url, app_language, active, blocked_reason, blocked_until, created_at"
    )
    .eq("role", "user")
...
export async function updateAppUserProfile(userId, patch) {
  const { error } = await supabase
    ...
    .update({
      ...
      gender: patch.gender,
      gender_custom: patch.genderCustom,
...
export async function loadCollaborators() {
  const { data, error } = await supabase.from("profiles").select("id, email, name, last_name, birth_date, country, main_language, avatar_url, role, active, can_moderate").order("name");
  if (error) {
...
export async function updateCollaboratorProfile(userId, { firstName, lastName, birthDate, role, active, canModerate, avatarUrl, country, mainLanguage }) {
  const { error } = await supabase
    .from("profiles")
    .update({ name: firstName, last_name: lastName, birth_date: birthDate || null, role, active, can_moderate: canModerate, avatar_url: avatarUrl, country, main_language: mainLanguage })
    .eq("id", userId);
  if (error) return { error: error.message };
  return { ok: true };
}
...
export async function loadDrinksDirectory() {
  ...
  // reste au format code brut pour les autres usages, ex. la page "Produits"). La nationalité,
  // elle, reste au format code brut : COUNTRY_ISO_CODES (utilisé pour retrouver le drapeau) est
  // lui-même indexé par ces codes, pas par le libellé français.
...
function rowToDrink(row) {
  return {
    ...
    defaultPriceEuro: row.default_price_euro,
    defaultPriceJeton: row.default_price_jeton,
...
function drinkToRow(d, partial = false) {
  const row = {
    ...
    default_price_euro: d.defaultPriceEuro,
    default_price_jeton: d.defaultPriceJeton,
