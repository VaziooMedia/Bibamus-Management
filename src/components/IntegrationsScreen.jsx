...
const INTEGRATIONS = [
  {
    ...
    where: "Variable d'environnement du déploiement (VITE_GEOAPIFY_API_KEY).\nModifiable uniquement en la changeant là où l'app est déployée, puis en redéployant.",
  ...
  },
  {
    ...
    where: "Secret d'une vraie fonction serveur (Edge Function google-place-search).\nGéré depuis le tableau de bord Supabase (Edge Functions → Secrets), jamais exposé au client.",
...
export function IntegrationsScreen() {
  return (
    <div>
      ...
      <p style={{ fontSize: "12.5px", color: "#8792A6", margin: "12px 0 20px", maxWidth: "600px", whiteSpace: "pre-line" }}>
        {"Aucune vraie clé n'est affichée ni modifiable ici.\nPar vraie sécurité, une clé secrète utilisée côté serveur n'est jamais exposée au client, et une clé utilisée côté client est figée au moment du déploiement."}
      </p>
        ...
        {INTEGRATIONS.map((i) => (
          <div key={i.name} style={{ background: "#16273D", border: "2px solid #28405C", borderRadius: "10px", padding: "16px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              <span style={{ fontSize: "14px", fontWeight: 700, color: "#F2F2E8", display: "flex", alignItems: "center", gap: "8px" }}>
                <span style={{ width: "3px", height: "13px", background: "#39FF66", borderRadius: "1px", flexShrink: 0, display: "inline-block" }} />
                {i.name}
              </span>
            ...
            <p style={{ fontSize: "11.5px", color: "#8792A6", margin: 0, whiteSpace: "pre-line" }}>{i.where}</p>
          </div>
        ))}
      </div>
    </div>
