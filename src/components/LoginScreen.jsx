...
export function LoginScreen({ onUnlock }) {
  ...
  const submit = async (e) => {
    ...
    const { data: profile, error: profileError } = await supabase.from("profiles").select("role, active, blocked_until, can_moderate, name, last_name, avatar_url").eq("id", data.user.id).single();
    setLoading(false);
    ...
    onUnlock(profile.role, profile.can_moderate, data.user.id, profile.name, profile.last_name, profile.avatar_url);
  };
